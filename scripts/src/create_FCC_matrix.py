...
UPPER_IX = 1000
PHYSIOLOGY = 'WT'
REACTIONS = ['biomass', 'TMDS', 'HEX1']  # Example reactions, adjust as needed
...
path_to_fcc_df = os.path.abspath(os.path.join(base_dir, config['paths']['path_to_fcc_df_{PHYSIOLOGY}']))
...
# Get FCC matrix for selected reactions
rxn_fcc = get_fccs_parallel(range(LOWER_IX, UPPER_IX), reactions=REACTIONS, path=path_to_fcc, n_cores=int(ncpu//2))

# Remove outliers with IQR
print('Removing outliers from FCC data...')
for rxn in REACTIONS:
    rxn_fcc[rxn] = remove_outliers_parallel(rxn_fcc[rxn])

# Save the FCC data
print('Saving FCC data...')
for rxn in REACTIONS:
    rxn_fcc[rxn].to_csv(path_to_fcc_df.format(rxn))