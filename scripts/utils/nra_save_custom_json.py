...
def nra_model_to_dict(model, sort=False):
    ...
    obj['concentration_control_coefficients'] = model.concentration_control_coefficients.to_dict().copy()
    obj['flux_control_coefficients'] = model.flux_control_coefficients.to_dict().copy()
    obj['reference_solution'] = model.reference_solution.to_dict().copy()
...
def nra_model_from_dict(obj, solver=None, custom_hooks=None):
    ...
    for the_var_dict in tqdm(obj['variables'], desc='Adding variables'):
    ...
    for the_cons_dict in tqdm(obj['constraints'], desc='Adding constraints'):
    ...
    nra_model.concentration_control_coefficients = pd.DataFrame(obj["concentration_control_coefficients"])
    nra_model.concentration_control_coefficients.index.name = 'concentration'
    nra_model.concentration_control_coefficients.columns.name = 'parameter'
    nra_model.flux_control_coefficients = pd.DataFrame(obj["flux_control_coefficients"])
    nra_model.flux_control_coefficients.index.name = 'flux'
    nra_model.flux_control_coefficients.columns.name = 'parameter'
    nra_model.reference_solution = pd.Series(obj["reference_solution"])
...
def save_json_nra_model(model, filepath):
    """
    ...
    obj = nra_model_to_dict(model)

    with open(filepath, 'w') as fid:
        json.dump(obj, fid, cls=MyEncoder)

def load_json_nra_model(filepath, solver = None):
    """
    ...
    model = nra_model_from_dict(obj, solver=solver)
    return model