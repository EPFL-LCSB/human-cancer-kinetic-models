...
from NRAplus.core.variables import *
from NRAplus.core.constraints import *
...
class NRAmodel_parallel(Model, LCSBModel):
    """
    NRA Model class
    ...
    def _prepare_enzymes(self, enzymes_to_regulate):
        ...
        for rxn in tqdm(self.reactions, desc='Adding NRA constraints to enzymes'):
    ...
    def _prepare_metabolites(self, reference_concentrations, concentration_control_coefficients):
        ...
        for met in tqdm(self.metabolites, desc='Adding NRA constraints to metabolites'):
    ...
    def prepare_metabolites(model, reference_concentrations, concentration_control_coefficients):
        """
        This method adds all the new NRA constraints/variables attached to a metabolite. e.g. Concentration Response
        Balance constraints, LogConcentrationRatio variables etc.
        ...
        for met in model.metabolites:
            M = model.add_variable(kind=LogConcentrationRatio,
                                    hook=met,
                                    lb=-MAX_FOLD_CHANGE,
                                    ub=MAX_FOLD_CHANGE,
                                    queue=False)
            ...
            if sanitized_id in concentration_control_coefficients.index:
                # Create the Concentration Response Balance constraint for the given metabolite
                control_coefficients = concentration_control_coefficients.loc[sanitized_id]
                model._create_response_balance_constraint(var=met,
                                                            nra_var=M,
                                                            control_coefficients=control_coefficients,
                                                            nra_constraint_type = ConcentrationResponseBalance)

                # Couple the concentration ratio to log concentration: ln(x_i) = LCR_i + ln(x_ref,i)
                lc = model.log_concentration.get_by_id(met.id).variable
                expression = lc - M - np.log(reference_concentrations.loc[met.id])
                model.add_constraint(kind=ConcentrationVariableCoupling,
                                    hook=met,
                                    expr=expression,
                                    lb=0,
                                    ub=0)
    ...
    def _prepare_reactions(self, reference_fluxes, flux_control_coefficients, type=NET, displacements=None):
        ...
        for rxn in tqdm(self.reactions, desc='Adding NRA constraints to reactions'):
    ...
    def _create_response_balance_constraint(self, var, nra_var, control_coefficients, nra_constraint_type, kind=NET):
        ...
        con  = self.add_constraint(kind=nra_constraint_type,
                            hook=var,
                            expr=expression,
                            lb=0,
                            ub=0)
    ...
    def _prepare_thermodynamics(self, net_reference_fluxes, type=NET):
        ...
        if type == NET:
            ...
            for rxn_id in tqdm(rxns_with_delta_g, desc='Setting deltaG bounds'):
...
def _init_worker_met(model, reference_concentrations, concentration_control_coefficients):
    """Initialize a global model object for multiprocessing in each worker."""
    global _model
    global _reference_concentrations
    global _ccc

    # Create a copy of the model to avoid pickling issues
    _model = model.copy()  # Assuming `model.copy()` exists
    _reference_concentrations = reference_concentrations
    _ccc = concentration_control_coefficients


from pytfa.io.dict import var_to_dict, cons_to_dict
from tqdm.auto import tqdm
import os
import json
from pytfa.io.json import MyEncoder
def _process_metabolite(met_id):
    """Worker function to process each metabolite and return a dictionary of new variables and constraints."""
    global _model
    global _reference_concentrations
    global _ccc

    met = _model.metabolites.get_by_id(met_id)
    
    new_vars = []
    new_constraints = []

    M = _model.add_variable(
        kind=LogConcentrationRatio,
        hook=met,
        lb=-MAX_FOLD_CHANGE,
        ub=MAX_FOLD_CHANGE,
        queue=False
    )
    new_vars.append(var_to_dict(M))

    sanitized_id = sanitize_cobra_vars(met.id)

    if sanitized_id in _ccc.index:
        control_coefficients = _ccc.loc[sanitized_id]

        # Create the associated Response Balance constraint
        expression = M

        for enzyme in _model.reactions:
            vmax = 'vmax_forward_' + enzyme.id

            if vmax in control_coefficients.index:
                enzyme_upregulation_var = _model.variables['EU_' + enzyme.id]
                enzyme_downregulation_var = _model.variables['ED_' + enzyme.id]
                expression -= control_coefficients.loc['vmax_forward_'+enzyme.id] * \
                              (enzyme_upregulation_var- enzyme_downregulation_var)  # TODO: Please check this!

        constraint  = _model.add_constraint(kind=ConcentrationResponseBalance,
                            hook=met,
                            expr=expression,
                            lb=0,
                            ub=0)

        new_constraints.append(cons_to_dict(constraint))

        lc = _model.log_concentration.get_by_id(met.id).variable
        expression = lc - M - np.log(_reference_concentrations.loc[met.id])
        
        constraint = _model.add_constraint(
            kind=ConcentrationVariableCoupling,
            hook=met,
            expr=expression,
            lb=0,
            ub=0
        ...
        obj = {"variables": new_vars, "constraints": new_constraints}
        # Save the object to a JSON file
        filepath = './tmp_met_json_MUT/{}.json'.format(met.id)
        ...
        print(f"Metabolite {met.id} does not have an associated control coefficient")
...
def prepare_metabolites(model, reference_concentrations, concentration_control_coefficients, processes=4):
    """
    This method parallelizes the addition of new NRA constraints/variables to a metabolic model.
    
    :param model: The metabolic model
    :param reference_concentrations: a pandas series of reference concentrations
    :param concentration_control_coefficients: a dataframe of control coefficients
    :param processes: Number of processes for parallel execution
    :return: A new model with all updated constraints and variables
    """

    metabolite_ids = [met.id for met in model.metabolites]

    if not os.path.exists('./tmp_met_json_MUT'):
        os.makedirs('./tmp_met_json_MUT')

    ctx = get_context("fork")  # Use "fork" for multiprocessing

    with ctx.Pool(processes, initializer=_init_worker_met, initargs=(model, reference_concentrations, concentration_control_coefficients)) as pool:
        results = pool.imap_unordered(_process_metabolite, metabolite_ids)

        for rerult in results:
            pass

    # tot_obj = {"variables": [], "constraints": []}
    # # Read all the json files inside the tmp_met_json folder and add them to the tot_obj
    # for filename in os.listdir('./tmp_met_json_MUT'):
    #     with open('./tmp_met_json_MUT/' + filename, 'r') as fid:
    #         obj = json.load(fid)
    #         tot_obj["variables"].extend(obj["variables"])
    #         tot_obj["constraints"].extend(obj["constraints"])
    
    # # Save the tot_obj to a JSON file
    # with open('./tmp_met_json_MUT/tot_obj.json', 'w') as fid:
    #     json.dump(tot_obj, fid, cls=MyEncoder)
...
def _init_worker(model, reference_fluxes, flux_control_coefficients, type, displacements):
    """Initialize a global model object for multiprocessing in each worker."""
    global _model
    global _reference_fluxes
    global _flux_control_coefficients
    global _type
    global _displacements

    _model = model.copy()  # Assuming model.copy() creates a new model copy
    _reference_fluxes = reference_fluxes
    _flux_control_coefficients = flux_control_coefficients
    _type = type
    _displacements = displacements


def _process_reaction(rxn_id):
    """Worker function to process each reaction and return a dictionary of new variables and constraints."""
    global _model
    global _reference_fluxes
    global _flux_control_coefficients
    global _type
    ...
    if _type == "NET":
        if rxn.id in _flux_control_coefficients.index:
            LFR = _model.add_variable(kind=LogFluxRatio,
                                      hook=rxn,
                                      lb=-MAX_FOLD_CHANGE,
                                      ub=MAX_FOLD_CHANGE,
                                      queue=False)
            LF = _model.add_variable(kind=LogFlux,
                                     hook=rxn,
                                     lb=-MAX_FOLD_CHANGE,
                                     ub=MAX_FOLD_CHANGE,
                                     queue=False)

            # _model._NET_LOG_FLUX[rxn.id] = LF

            control_coefficients = _flux_control_coefficients.loc[rxn.id]
            ...
            constraint  = _model.add_constraint(kind=FluxResponseBalance,
                                hook=rxn,
                                expr=expression,
                                lb=0,
                                ub=0)
            ...
            # Couple the flux ratio to log flux: ln(v_i) = LFR_i + ln(x_ref,i)
            if _reference_fluxes.loc[rxn.id] > 0:
                expression = LF - LFR - np.log(_reference_fluxes.loc[rxn.id])
            else:
                expression = LF - LFR - np.log(-_reference_fluxes.loc[rxn.id])

            constraint = _model.add_constraint(kind=FluxVariableCoupling,
                                              hook=rxn,
                                              expr=expression,
                                              lb=0,
                                              ub=0)
            new_constraints.append(cons_to_dict(constraint))
            new_vars.append(var_to_dict(LFR))
            new_vars.append(var_to_dict(LF))
        
            # Save the object to a JSON file
            obj = {"variables": new_vars, "constraints": new_constraints}
            filepath = './tmp_rxn_json/{}.json'.format(rxn.id)
            ...
            print(f"Flux {rxn.id} does not have an associated control coefficient")

    return rxn_id, True


def prepare_reactions(model, reference_fluxes, flux_control_coefficients, type="NET", displacements=None, processes=4):
    """
    This method parallelizes the addition of new NRA constraints/variables to reactions.

    :param model: The metabolic model
    :param reference_fluxes: a pandas series of reference fluxes
    :param flux_control_coefficients: a dataframe of flux control coefficients
    :param type: NET or SPLIT for net or fwd/bwd control coefficients
    :param displacements: Thermo displacement data for the SPLIT type to determine FWD/BWD fluxes
    :param processes: Number of processes for parallel execution
    :return: A new model with all updated constraints and variables
    ...
    # Get the reaction IDs to process
    reaction_ids = [rxn.id for rxn in model.reactions]

    # Create a folder to store the JSON files
    if not os.path.exists('./tmp_rxn_json'):
        os.makedirs('./tmp_rxn_json')

    # Use multiprocessing to parallelize the processing of reactions
    ctx = get_context("fork")  # Use "fork" for multiprocessing

    with ctx.Pool(processes, initializer=_init_worker, initargs=(model, reference_fluxes, flux_control_coefficients, type, displacements)) as pool:
        results = pool.imap_unordered(_process_reaction, reaction_ids)
    ...
    # tot_obj = {"variables": [], "constraints": []}
    # # Read all the json files inside the tmp_rxn_json folder and add them to the tot_obj
    # for filename in os.listdir('./tmp_rxn_json'):
    #     with open('./tmp_rxn_json/' + filename, 'r') as fid:
    #         obj = json.load(fid)
    #         tot_obj["variables"].extend(obj["variables"])
    #         tot_obj["constraints"].extend(obj["constraints"])
    
    # # Save the tot_obj to a JSON file
    # with open('./tmp_rxn_json/tot_obj.json', 'w') as fid:
    #     json.dump(tot_obj, fid, cls=MyEncoder)

    return True


def add_vars_cons_from_dict(model, local_dict):
    
    # Create a new model and merge all changes
    ...
    for the_var_dict in tqdm(local_dict["variables"], desc='Updating model vars'):
    ...
    for the_cons_dict in tqdm(local_dict['constraints'], desc='Updating model cons'):
